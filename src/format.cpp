...
Format::Format(igzstream & infile, Header & _header, std::uint32_t len, std::uint32_t n_fmt) {
  header = &_header;
  if (len == 0) {
    return;
  }
  
  std::cout << "reading format: " << len << std::endl;
  // read the sample data into a buffer, but don't parse until required
  buf = new char[len];
  infile.read(reinterpret_cast<char *>(&buf[0]), len);
  
  // read the keys available
  ...
  for (std::uint32_t i = 0; i < n_fmt; i++ ){
    ...
    std::cout << "format_type: " << type_val.type << ", size: " << type_val.type_size << ", len: " << type_val.n_vals << std::endl;
    format_idx = get_int(buf, buf_idx, type_val.type_size);
    std::cout << "format_idx: " << format_idx << std::endl;
